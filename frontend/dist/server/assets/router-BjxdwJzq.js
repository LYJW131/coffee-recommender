...
const $$splitComponentImporter = () => import("./index-BZGCBA7s.js");
...
const Route2 = createFileRoute("/api/recommend")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const apiKey = process.env.GEMINI_API_KEY;
        const model = process.env.GEMINI_MODEL || "gemini-flash-lite-latest";
        const endpoint = process.env.GEMINI_ENDPOINT || `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`;
        if (!apiKey) {
...
const ApiRecommendRoute = Route2.update({
  id: "/api/recommend",
  path: "/api/recommend",
  getParentRoute: () => Route$2
});
